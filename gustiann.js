...
const vcard = 'BEGIN:VCARD\n'  
            ...
            + 'FN: DAFFA N\n'  
            + 'ORG: OWNER DAFFA BOT;\n'  
            + 'TEL;type=CELL;type=VOICE;waid=6288239430315:+6288239430315\n' 
            + 'END:VCARD' 
...
const ownerNumber = ["6288239430315@s.whatsapp.net"] 
...
// ANTI LINK GRUP
		...
		reply(`Link Group Terdeteksi maaf ${sender.split("@")[0]} anda akan di kick dari Group, Karena Tidak Memenuhi persyaratan Kami, Chat Owner kami : wa.me/6288239430315`)
		setTimeout( () => {
            ...
            if (bad.includes(messagesC)) {
                if (!isGroupAdmins) {
                    return reply("JAGA UCAPAN DONG!! 😠")
                        ...
                        .then(() => {
                            ahmad.sendMessage(from, `*「 ANTI BADWORD 」*\nKamu dikick karena berkata kasar!!, Chat Owner Kami Dan Minta Maaf Lah : wa.me/6288239430315`, text ,{quoted: mek})
...
//CASE BADWORD
	...
	if (!isRegistered) return reply(ind.noregis())
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname))
				...
				ll3 = ct.split("|")[2];
				reply(`[❕] Loading`)
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker?text=${ll1}&text2=${ll2}&text3=${ll3}&theme=google-suggestion&apikey=ahmadbotwa`)
		...
		if (isBanned) return reply(ind.baned())
                ...
                ll2 = ct.split("|")[1];
				reply(ind.wait())
				anu = await fetchJson(`https://dapuhy-apikey.herokuapp.com/api/textmaker/game?text=${ll1}&text2=${ll2}&theme=pubg&apikey=ahmadbotwa`)
				buffer = await getBuffer(anu.result.url)
				ahmad.sendMessage(from, buffer, image, {quoted: { key: { fromMe: false, participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "status@broadcast" } : {}) }, message: { "imageMessage": { "url": "https://mmg.whatsapp.net/d/f/At0x7ZdIvuicfjlf9oWS6A3AR9XPh0P-hZIVPLsI70nM.enc", "mimetype": "image/jpeg", "caption": "DapBOT~", "fileSha256": "+Ia+Dwib70Y1CWRMAP9QLJKjIJt54fKycOfB2OEZbTU=", "fileLength": "28777", "height": 1080, "width": 1079, "mediaKey": "vXmRR7ZUeDWjXy5iQk17TrowBzuwRya0errAFnXxbGc=", "fileEncSha256": "sR9D2RS5JSifw49HeBADguI23fWDz1aZu4faWG/CyRY=", "directPath": "/v/t62.7118-24/21427642_840952686474581_572788076332761430_n.enc?oh=3f57c1ba2fcab95f2c0bb475d72720ba&oe=602F3D69", "mediaKeyTimestamp": "1610993486", "jpegThumbnail": fs.readFileSync('Agan/odc.jpeg')} } }, caption: 'Nih hasilnya kak...'})
				break  
					case 'cml':
...
break
		...
		try {
		ppimg = await ahmad.getProfilePicture(`${sender.split('@')[0]}@c.us`)
		} catch {
		ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
		}
                veri = sender
                if (isGroup) {
                    ...
                    await ahmad.sendMessage(from, ppimg, image, {quoted: mek, caption: ind.registered(namaUser, umurUser, serialUser, time, sender)})
	...
	case 'hobby':
		...
		break
				...
				if (isBanned) return reply(ind.baned())
                    ...
                    wew = fs.readFileSync(`./Agan/logo.jpg`)
                    Agan = `
꧁𝚂𝙸𝙼𝙿𝙻𝙴 𝚆𝙷𝙰𝚃𝚂𝙰𝙿𝙿 𝙱𝙾𝚃꧂                    
...
│►𝙎𝙩𝙖𝙩𝙪𝙨: 𝘼𝙘𝙩𝙞𝙫𝙚
│►𝙏𝙤𝙩𝙖𝙡: 𝙁𝙪𝙡𝙡 𝙁𝙞𝙩𝙪𝙧
│►𝙎𝙥𝙚𝙚𝙙: ${process.uptime()}
│►𝙑𝙚𝙧𝙨𝙞 𝙒𝘼: ${ahmad.user.phone.wa_version}
│►𝙎𝙚𝙧𝙫𝙚𝙧: ${ahmad.browserDescription[0]}
│►𝘽𝙧𝙤𝙬𝙨𝙚𝙧: ${ahmad.browserDescription[1]}
╰──────────────────
...
╭───────────────────    
│❧ 𝙉𝙖𝙢𝙖: ${pushname}
│❧ 𝙉𝙤𝙢𝙚𝙧: wa.me/${sender.split("@")[0]}
│❧ 𝙐𝙖𝙣𝙜𝙈𝙪: Rp${uangkau}
│❧ 𝙓𝙋: ${getLevelingXp(sender)}/${reqXp}
│❧ 𝙇𝙚𝙫𝙚𝙡: ${getLevelingLevel(sender)}
│❧ 𝙐𝙨𝙚𝙧 𝙍𝙚𝙜𝙞𝙨𝙩𝙚𝙧: ${_registered.length}
╰───────────────────
...
` 
                    ahmad.sendMessage(from, wew, image, { quoted: mek, caption: Agan })
					break
				case 'donasi':
				case 'donate':
				if (!isRegistered) return reply(ind.noregis())
		...
		reply(ind.wait)
                ...
                case 'ytmp4':
                    ...
                    ahmad.sendMessage(from, get_audio, video, { mimetype: 'video/mp4', filename: `${get_result.title}.mp4`, quoted: mek })
                    break
                case 'darkneon':
                case 'candlemug':
                case 'lovemsg':
		...
		if (isBanned) return reply(ind.baned())
			...
			if (isBanned) return reply(ind.baned)    
				...
				if (!isRegistered) return reply(ind.noregis)
                 ...
                 tod = await getBuffer(`https://i.ibb.co/WpCKMHZ/logo.jpg`)
		...
		if (budy.includes(`DAFFA`)) {
                const DAFFA = fs.readFileSync('./sound/toxic');
                ahmad.sendMessage(from, DAFFA, MessageType.audio, {quoted: mek})
                  }
                  
       if (budy.includes(`@6288239430315`)) {
                  reply(`Kenapa Panggil Owner Ku!! ${pushname}`)
                  }
                  
                   if (budy.includes(`@6281216374929`)) {
                  reply(`Kenapa Panggil Hafidz Dia Temen Ownerku!! ${pushname}`)
                  }
                  
         if (budy.includes(`@6289685758250`)) {
                  reply(`Iya?? Ada Apa Kok Manggil-Manggil!! ${pushname}`)
                  }
		...
		if (budy.includes(`bot`)) {
                  reply(`Iya Ada Yang Bisa Saya Bantu??, ketik ${prefix}help Ya *${pushname}*`)
                  }
		...
		if (budy.includes(`Bot`)) {
			...
			if (isGroup && !isCmd && isSimi && budy != undefined) {
						...
						console.log(color('DAFFA','yellow'), ('Command Tidak Ditemukan!!','yellow'), color(sender.split('@')[0]))
					}
					}
		} catch (e) {
			console.log('Error : %s', color(e, 'red'))
